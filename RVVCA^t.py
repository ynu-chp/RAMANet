...
def generate_all_deterministic_alloc(n_agents, m_items,unit_demand = False) -> torch.tensor: 
...
def test_time_forward( input_bids: torch.tensor, input_values: torch.tensor,t):
    """
    ...
    input_values= torch.cat((input_values, t*torch.ones(B, 1, m)), axis=1)
    input_bids = torch.cat((input_bids, torch.zeros(B, 1, m)), axis=1)
    n=n+1
    ...
    return utility[:-1,], payments[:-1,], (input_bids * item_allocation).sum(-1).permute(1,0)[:-1,]
...
l=(test_values.clone().detach()-test_bids.clone().detach()).min()
h=(test_values.clone().detach()-test_bids.clone().detach()).max()
print("l:",l,"h:",h)
for num in range(int(test_values.shape[0] / bs)):
    ...
    RVVCA_utility = torch.zeros(1)
    RVVCA_payment = torch.zeros(1)
    RVVCA_cost = torch.zeros(1)
    idx=0
    for i in range(int(math.log2(h/l))+1):
        utility, payment,  cost = test_time_forward(
            test_bids[num * bs:(num + 1) * bs].clone().detach() ,
            test_values[num * bs:(num + 1) * bs].clone().detach(),l*(2**i))
        RVVCA_utility += utility.sum()
        RVVCA_payment += payment.sum()
        RVVCA_cost += cost.sum()
        idx+=1
    test_utility += RVVCA_utility/idx
    test_payment += RVVCA_payment/idx
    test_cost += RVVCA_cost/idx
